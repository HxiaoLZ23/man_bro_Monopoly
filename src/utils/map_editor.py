            ...
            try:
                ...
                if cmd == "quit":
                    ...
                    if len(command) >= 3:
                        x = int(command[1])
                        y = int(command[2])
                        self.toggle_junction(x, y)
                    else:
                        print("用法: junction <x> <y>")
...
class MapEditorGUI:
    ...
    def update_property_panel(self):
        ...
        # 岔路状态
        ttk.Label(self.property_content, text=f"岔路点: {'是' if cell.is_junction else '否'}").pack(anchor=tk.W)
        ...
        if path_index is not None:
            ttk.Label(self.property_content, text=f"路径索引: {path_index}").pack(anchor=tk.W)
