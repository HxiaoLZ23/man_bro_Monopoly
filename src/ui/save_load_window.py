...
class SaveLoadWindow:
    ...
    def draw_save_details(self):
        ...
        if self.selected_index >= 0 and self.selected_index < len(self.saves_list):
            ...
            details = [
                ...
                f"创建时间: {save_info['created_time'].strftime('%Y-%m-%d %H:%M:%S')}",
            ...
            for detail in details:
                if y_offset < details_rect.bottom - 20:
                    ...
                    for word in words:
                        test_line = line + word + " "
                        test_surface = self.font_small.render(test_line, True, self.colors['text'])
                        if test_surface.get_width() <= details_rect.width - 20:
                            line = test_line
                        else:
                            if line:
                                text_surface = self.font_small.render(line.strip(), True, self.colors['text'])
    ...
    def draw_input_dialog(self):
        ...
        # 绘制输入文字
        input_surface = self.font_medium.render(self.input_text + "|", True, self.colors['text'])
